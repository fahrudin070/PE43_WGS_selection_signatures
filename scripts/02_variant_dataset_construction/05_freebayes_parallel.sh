#!/usr/bin/env bash
# Script: 05_freebayes_parallel.sh
# Purpose: Run FreeBayes in parallel across genomic regions and merge variant calls into a single VCF stream.
# Input:
#   - regions file
#   - reference genome
#   - BAM alignment files
# Output:
#   - merged VCF stream from parallelized FreeBayes jobs
# Main tools:
#   - FreeBayes
#   - GNU parallel
# Notes:
#   - Adapted parallel calling wrapper used in the variant calling workflow
